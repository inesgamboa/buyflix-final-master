...
// sqlite3 -csv -header movies.sqlite3 'select "imdbID" as id, "title", "released", "poster" from movies' | csvtojson --maxRowLength=0 > movies.json

// Firebase
var Rebase = require('re-base')
var base = Rebase.createClass({
  apiKey: "AIzaSyA-DDRNH8bNWzqql8Ah5oDdLq13B0w5A1k",   // replace with your Firebase application's API key
  databaseURL: "https://buyflix-final-4f11d.firebaseapp.com", // replace with your Firebase application's database URL
})
...
var App = React.createClass({
  ...
  viewChanged: function(view) {
    ...
    if(view === 'latest'){
      return (
        this.setState({
          currentView: movieData.sort(this.movieCompareByReleased), className: "active"
        })
      )
    }
    if(view === 'alpha'){
      return(
        this.setState({
          currentView: movieData.sort(this.movieCompareByTitle), className: "active"
        })
      )
    }
    this.setState({
  ...
  renderMainSection: function() {
    if (this.state.currentView === 'map') {
      ...
      return (
        <div>
          <MovieList movies={this.state.movies} renderMovie={this.renderMovie} />
